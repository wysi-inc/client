...
const TopScoresPanel = (p: Props) => {
    ...
    const commonMods: string[] = useMemo(() => {
        ...
        p.best.map(score => score.mods.length > 0 ?
    ...
    if (p.best.length < 1) return (<div></div>);
    ...
    const lengths = p.best.map(score => calculateModifiedLength(score.beatmap.total_length, score.mods));
    const totalLength = lengths?.reduce((acc, length) => acc + length, 0);
    const averageLength = totalLength / p.best.length;

    const bpms = p.best.map(score => calculateModifiedBpm(score.beatmap.bpm, score.mods));
    const totalBpm = bpms?.reduce((acc, bpm) => acc + bpm, 0);

    const averageBpm = Math.round(totalBpm / p.best.length);
    const averageAcc = p.data.statistics.hit_accuracy;

    const averageCombo = Math.round(p.best.map(score => score.max_combo)?.reduce((a, b) => a + b, 0) / p.best.length);
    const averagePP = Math.round(p.best.map(sc => sc.pp ? parseInt(sc.pp) : 0)?.reduce((a, b) => a + b, 0) / p.best.length);
    ...
    return (
        <div className="grid grid-cols-2 gap-3 p-3 grow">
            ...
            <div className="col-span-2">
                <PpLine data={p.best} color={colors.ui.font} width={350} />
            </div>

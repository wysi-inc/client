...
const languages: LanguageName[] = [
    ...
    { code: "pt-br", name: "Brazilian Portuguese", nativeName: "Português(BR)" },
...
export function getLang(code: string) {
    ...
    const language = getLangName(code);
    if (!language) return ['', '', '', ''];
    return [code, flag, language.name, language.nativeName];
}
...
export function useCountTranslatedKeys(): [number, (code: string) => { progress: number, approval: number }] {
    ...
    async function getCount() {
        try {
            const res = await fina.get('/lang/progress');
            if (!res.ok) return;

...
const name = async (filters) => {
    ...
    if (filters) {
        if (filters.query)
            obj.q = filters.query;
        if (filters.general)
            obj.c = filters.general.toString().split(',').join('.');
        if (filters.mode)
            obj.m = _mode.indexOf(filters.mode).toString();
        if (filters.section)
            obj.s = filters.section;
        if (filters.nfsw)
            obj.nsfw = '';
        else
            obj.nsfw = false;
        if (filters.genre)
            obj.g = _genre.indexOf(filters.genre).toString();
        if (filters.language)
            obj.l = _language.indexOf(filters.language).toString();
        if (filters.include)
            obj.e = filters.include.toString().split(',').join('.');
        if (filters.rank)
            obj.r = filters.rank.toString().split(',').join('.');
        if (filters.sort)
